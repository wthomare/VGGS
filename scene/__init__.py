...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        for resolution_scale in resolution_scales:
            ...
            # xp
            # exclude cameras with 0 nearest_cam
            self.train_cameras[resolution_scale] = [cam for cam in self.train_cameras[resolution_scale] if len(cam.nearest_id) > 0]
            imgName_to_index = {cam.image_name: idx for idx, cam in enumerate(self.train_cameras[resolution_scale])}
            for cam in self.train_cameras[resolution_scale]:
                cam.nearest_id = [imgName_to_index[img_name] for img_name in cam.nearest_names]
