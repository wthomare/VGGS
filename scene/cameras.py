...
def process_image(image_path, resolution, ncc_scale, source_path=None, image_name=None):
    ...
    if len(image.split()) > 3:
        ...
        if source_path is not None:
            path_mask = os.path.join(source_path, 'mask', image_name + '.png')
            if os.path.exists(path_mask):
                mask_ = Image.open(path_mask)
                loaded_mask = PILtoTorch(mask_, resolution)
        gt_image = resized_image_rgb
        if ncc_scale != 1.0:
            ncc_resolution = (int(resolution[0]/ncc_scale), int(resolution[1]/ncc_scale))
...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
                 ...
                 ncc_scale=1.0, 
                 vggt_depth=None, mono_normal=None, segment=None, edge=None, depth_conf=None, source_path=None, match=None,
        ...
        if self.preload_img:
            gt_image, gray_image, loaded_mask = process_image(self.image_path, self.resolution, ncc_scale, source_path=self.source_path, image_name=self.image_name)
        ...
        save_debug_cameras = False
        if save_debug_cameras:
            scan_folder = '/data12_1/xp/data/DTU/set_22_25_28/scan24/dense'
            # scan_folder = '/data12_1/xp/code/vggt/scan24'
            path_save = os.path.join(scan_folder, 'cam_debug')
            if not os.path.exists(path_save):
                os.makedirs(path_save)
            img_names = ['0000', '0001', '0002']
            if image_name in img_names:
                cam_save_path = os.path.join(path_save, image_name)
                ...
                np.save(os.path.join(cam_save_path, 'R.npy'), R)
                np.save(os.path.join(cam_save_path, 'T.npy'), T)

                rays = self.get_rays().detach().cpu().numpy()
                np.save(os.path.join(cam_save_path, 'rays.npy'), rays)
                np.save(os.path.join(cam_save_path, 'world_view_transform.npy'), self.world_view_transform.detach().cpu().numpy())
                np.save(os.path.join(cam_save_path, 'intrisics.npy'), intr)

        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0,1).cuda()
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        # to determinstic
        # self.full_proj_transform = torch.einsum('bij,bjk->bik', self.world_view_transform.unsqueeze(0), self.projection_matrix.unsqueeze(0)).squeeze(0)
