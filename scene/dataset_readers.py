...
class CameraInfo(NamedTuple):
    ...
    fx: float
    fy: float
    vggt_depth: np.array = None
    depth_conf: np.array=None
    mono_normal: np.array = None
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, scan_folder=None):
    cam_infos = []

    if scan_folder is not None:
        depths_folder = os.path.join(scan_folder, "depth_vggt")
        normals_folder = os.path.join(scan_folder, 'normal')

    else:
        depths_folder = images_folder
        normals_folder = images_folder
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        depth_conf_path = os.path.join(depths_folder, image_name + "_conf.npy")
        depth_conf = None

        normal_path = os.path.join(normals_folder, image_name + '_normal.npy')
        normal = None
        ...
        cam_info = CameraInfo(uid=uid, global_id=idx, R=R, T=T, FovY=FovY, FovX=FovX,
                              ...
                              vggt_depth=depth, mono_normal=normal, depth_conf=depth_conf)
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), scan_folder=path)
