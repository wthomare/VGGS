...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # xp get color
        # sfm_color = torch.tensor(np.asarray(pcd.colors)).float().cuda()
        # self.sfm_color = sfm_color
    ...
    def depths_to_points_color(self, fov_camera, depth, image, scale=1, mask=None):
        st = int(max(int(scale/2)-1,0))
        depth_view = depth.squeeze()[st::scale,st::scale]
        rays_d = fov_camera.get_rays(scale=scale)
        depth_view = depth_view[:rays_d.shape[0], :rays_d.shape[1]]
