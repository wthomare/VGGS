...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background, 
               app_model=None, max_depth=5.0, volume=None, use_depth_filter=False, tsdf_edge_keep_ratio=0.0):
    # box
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if use_depth_filter:
            ...
            depth_tsdf[mask] = 0
        if tsdf_edge_keep_ratio > 0:
            edge_mask = image_edge_mask(rendering, tsdf_edge_keep_ratio)
            depth_tsdf[~edge_mask] = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/tnt_10views.yaml")
